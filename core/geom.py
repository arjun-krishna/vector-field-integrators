...
class WindowGeometry:
    ...
    def transform_screen_coords(self, xs: int, ys: int) -> Tuple[int, int]:
        ...
        if self.on_grid(xs, ys):
            x = self.xrange[0] + ( self.range(self.xrange) / self.range(self.gridX) * (xs - self.gridX[0]))
            y = self.yrange[0] + ( self.range(self.yrange) / self.range(self.gridY) * (self.gridY[1] - ys))
            return (x, y)
        else:
            return None
    ...
    def transform_coords(self, x: int, y: int, clip: bool = True) -> Tuple[int, int]:
        ...
        if self.in_range(x, y):
            xs = self.gridX[0] + (self.range(self.gridX) / self.range(self.xrange) * (x - self.xrange[0]))
            ys = self.gridY[1] + (self.range(self.gridY) / self.range(self.yrange) * (self.yrange[0] - y))
            return (xs, ys)
        else:
            return None
